...
router.post("/save", async (req, res) => {
  try {
    ...
    await newPost.save();
    res.status(200).json({ success: "Restaurant saved successfully" });
  } catch (err) {
...
router.put("/:id", async (req, res) => {
  try {
    ...
    res.status(200).json({ success: "Restaurant updated successfully" });
  } catch (err) {
    res.status(400).json({ error: err });
  }
});
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    res.status(200).json({ success: "Restaurant deleted successfully", deletedPost });
  } catch (err) {
    res.status(400).json({ error: err });
  }
});
